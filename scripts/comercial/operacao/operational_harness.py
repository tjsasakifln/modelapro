...
def _queue_cancel(root: Path) -> dict[str, Any]:
    ...
    try:
        runner.submit(first["job_id"], lambda: (time.sleep(0.1), {"outcome": "succeeded"})[1])
        runner.submit(second["job_id"], lambda: {"outcome": "succeeded"})
        runner.cancel(second["job_id"])
    finally:
        runner.shutdown(wait=True)
    state = store.get(second["job_id"])["state"]
    if state != "cancelled":
        raise AssertionError(f"queued cancellation produced {state!r}")
    return {"cancelled_job_state": state}
...
def _backup_restore(root: Path) -> dict[str, Any]:
    ...
    if restored.get(job["job_id"])["job_id"] != job["job_id"]:
        raise AssertionError("restored job identity differs")
    return {"backup": str(backup), "restored_job_id": job["job_id"]}
...
def run_harness(budget_path: Path, *, exercise: bool = False, corpus: list[str] | None = None) -> dict[str, Any]:
    ...
    evidence: dict[str, Any] = {
        ...
        "environment": {"platform": platform.platform(), "python": sys.version, "pid": os.getpid(), "free_disk_bytes": disk.free},
        "checks": {name: {"status": "NOT_RUN"} for name in CHECKS},
        "resource": {"elapsed_seconds": None, "max_rss_kib": _max_rss_kib(), "free_disk_bytes": disk.free, "exit_code": 0},
    }
    if not exercise:
        return evidence
...
def main(argv: list[str] | None = None) -> int:
    ...
    evidence["resource"] = evidence.get("resource", {}) | {"elapsed_seconds": round(time.monotonic() - started, 6)}
