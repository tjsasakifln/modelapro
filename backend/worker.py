...
def _sample_from_prepared(
    ...
    if isinstance(row_ledger, Mapping):
        ...
        observed = sum(
            ...
            if isinstance(row, Mapping) and row.get("observed_target")
        )
...
def _preview_jsonable(value: Any) -> Any:
    """JSON-safe cell for preview payloads. Does not invent numeric values."""
    if value is None:
        return None
    ...
    if isinstance(value, float):
        if value != value:  # NaN
            return None
        return float(value) if value not in (float("inf"), float("-inf")) else None
    if isinstance(value, str):
        return value
    if isinstance(value, Mapping):
        return {str(k): _preview_jsonable(v) for k, v in value.items()}
    if isinstance(value, (list, tuple)):
        return [_preview_jsonable(v) for v in value]
    ...
    try:
        ...
        if isinstance(value, np.generic):
            return _preview_jsonable(value.item())
        if isinstance(value, np.ndarray):
            return [_preview_jsonable(v) for v in value.tolist()]
    except Exception:
        pass
    try:
        if value != value:
...
def _preview_skip_column(original: str, internal: str) -> bool:
    orig = (original or "").strip()
    intern = (internal or "").strip()
    if not orig and not intern:
        return True
    lowered = intern.lower()
    return lowered.startswith("unnamed") or intern.startswith("_unnamed")
...
def _preview_column_profile(bundle: Any, spec: Mapping[str, Any]) -> dict:
    ...
    for entry in entries:
        ...
        if _preview_skip_column(original, internal):
            continue
        ...
        if series is not None:
            if pd.api.types.is_datetime64_any_dtype(series):
                ...
                unique = [v for v in series.dropna().tolist()]
                nunique = len({str(v) for v in unique})
                if nunique > 0 and nunique <= 32:
                    kind = "categorical"
                    categories = sorted({str(v) for v in unique})
                    groups[internal] = {"columns": [], "base_variable": original or internal}
                else:
                    kind = "text"
        role = roles.get(original) or roles.get(internal)
        columns[internal] = {
            "original_name": original or internal,
            "role": role,
            "kind": kind,
            "unit": units.get(original) or units.get(internal),
            "group_id": internal if kind == "categorical" else None,
            "categories": categories,
            "reference_category": None,
        }
        sample_raw: List[Any] = []
        sample_interpreted: List[Any] = []
        if original and original in raw_frame.columns:
            sample_raw = [_preview_jsonable(v) for v in raw_frame[original].head(5).tolist()]
        elif internal in raw_frame.columns:
            sample_raw = [_preview_jsonable(v) for v in raw_frame[internal].head(5).tolist()]
        if series is not None:
            sample_interpreted = [_preview_jsonable(v) for v in series.head(5).tolist()]
        ui_column_map[internal] = {
            "original_name": original or internal,
            "kind": kind,
            "unit": units.get(original) or units.get(internal),
            "role_suggestion": role,
            "sample_raw": sample_raw,
            "sample_interpreted": sample_interpreted,
            "categories": categories,
        }
    ...
    if not frame.empty:
        ...
        for _, rec in preview_df.iterrows():
            sample_preview.append(
                {str(k): _preview_jsonable(rec[k]) for k in preview_df.columns}
            )
    ledger = _get(bundle, "row_ledger")
    if isinstance(ledger, (list, tuple)):
        row_ledger = [_preview_jsonable(item) for item in ledger]
    else:
        row_ledger = _preview_jsonable(_as_dict(ledger) or [])
    return {
        "column_map": ui_column_map,
        "feature_schema": {
...
def compose_preview(
    *,
    ...
    # observed kinds from parsed_frame, not a fitted encoder.
    ...
    return {
        ...
        "target_col": request_spec.get("target_col") or "",
