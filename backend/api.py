...
def preview_peer_issues(peers: Optional[Mapping[str, Any]] = None) -> List[dict]:
    """Structured issues when a required preview peer is missing. Empty if ready."""
    bound = dict(peers) if peers is not None else get_peers()
    missing = missing_peers(bound, REQUIRED_PREVIEW_PEERS)
    if not missing:
        return []
    return [
        make_issue(
...
def preview_ready(peers: Optional[Mapping[str, Any]] = None) -> bool:
    return not preview_peer_issues(peers)
...
def _preview_unavailable_response(issues: List[dict]) -> JSONResponse:
    return _issues_response(
        503,
        "ingest_market unavailable",
        issues,
        extra={"status": "not_ready", "integration": "INTEGRATION_PENDING"},
    )


@app.get("/health")
async def health_check():
    issues = preview_peer_issues()
    if issues:
        return _preview_unavailable_response(issues)
    return {"status": "healthy"}


@app.get("/ready")
async def ready_check():
    """Fail closed when a required preview peer is missing."""
    issues = preview_peer_issues()
    if issues:
        return _preview_unavailable_response(issues)
    return {"status": "ready", "preview_ready": True}
...
async def preview(
    ...
    peers = get_peers()
    missing_issues = preview_peer_issues(peers)
    if missing_issues:
        return _preview_unavailable_response(missing_issues)
    ...
    try:
        ...
        return _preview_unavailable_response(list(exc.issues))
    except Exception as exc:
