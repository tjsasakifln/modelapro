...
| C01-A03 | Grau pedido sem perda ou falso aceite | IMPLEMENTED_VERIFIED | Grau 3 com ajuste válido devolve análise (`not_met`/`pending`), não `NO_WINNER`. Grau 1 com `declared_documentary`+proveniência sintética atinge `grade_requirement_status=met` e `review_required`/`ready_for_professional_signoff`. Conflito de aliases → `GRADE_ALIAS_CONFLICT`. Input corrompido → `CompositionError` com código estável. |
...
| C01-A08 | API ouro e recusa | IMPLEMENTED_VERIFIED | TestClient FastAPI, peers reais. POST/GET duas vezes ouro (ponto igual, snapshot MP/1). GET `/artifacts/frozen_project.json` 200 com `model_state` e `value.point` igual ao snapshot; GET `evidence_bundle.zip` 200 (PK). Inapto `candidate_cols=[]` → 400 `CANDIDATE_COLS_EMPTY`. Token inválido → 403. Fingerprint divergente → 409. |
