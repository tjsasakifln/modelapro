...
| command | exit |
|---|---|
| `python3 -m pytest tests/c11_persistence/ -q` (run 1) | 0 (37 passed) |
| `python3 -m pytest tests/c11_persistence/ -q` (run 2) | 0 (37 passed) |
...
- **C11-A02.** Reopening the store reloads immutable revisions. Abandoned `running` becomes `interrupted` and never `succeeded` without a stored snapshot. Complete idempotency key reuses; incomplete keys create a new job. No `resume_search`.
...
- **C11-A04.** Two connections for different jobs never cross. Light payloads omit PDF/snapshot/model_metrics. Dead `send_text` removes the socket and does not raise into the runner. Unscoped `send_notification` is dropped. `/ws` requires local token (not a cloud account).
