...
def fill_preview_spec(payload: Any) -> dict:
    """Preview RequestSpec: target_col may be empty or null; structural defaults only.
    ...
    POST /jobs still goes through validate_request_spec without this relaxation.
    """
    ...
    if not isinstance(search, Mapping):
        ...
        search = dict(search)
        if not search.get("mode"):
            search["mode"] = "preview"
        search.setdefault("budget", None)
        search.setdefault("objective", "none")
        search.setdefault("seed", 0)
        raw["search_policy"] = search
    evaluation = raw.get("evaluation_policy")
    if not isinstance(evaluation, Mapping):
        raw["evaluation_policy"] = {
            "method": EVALUATION_METHOD_NONE,
            "partitions": None,
            "groups": None,
            "seed": 0,
        }
        ...
        if not evaluation.get("method"):
            evaluation["method"] = EVALUATION_METHOD_NONE
        evaluation.setdefault("partitions", None)
        evaluation.setdefault("groups", None)
        evaluation.setdefault("seed", 0)
        raw["evaluation_policy"] = evaluation
...
def validate_request_spec(payload: Any, *, allow_empty_target: bool = False) -> dict:
    ...
    allow_empty_target is preview-only; POST /jobs must keep the default False.
    """
    issues: List[dict] = []
    try:
